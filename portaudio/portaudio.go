...
For the most part, these bindings parallel the underlying PortAudio API; please
refer to http://www.portaudio.com/docs.html for details.  Differences
introduced by the bindings are documented here:

Instead of passing a flag to OpenStream, audio sample formats are inferred from
the signature of the stream callback or, for a blocking stream, from the types
of the buffers.  See the StreamCallback and Buffer types for details.

Blocking I/O:  Read and Write do not accept buffer arguments; instead they use
the buffers (or pointers to buffers) provided to OpenStream.  The number of
samples to read or write is determined by the size of the buffers.

The StreamParameters struct combines parameters for both the input and the
output device as well as the sample rate, buffer size, and flags.
*/
package portaudio

/*
#cgo linux CFLAGS: -DPA_USE_ALSA=1 -Wno-deprecated-declarations
...
//#xxxcgo pkg-config: portaudio-2.0
...
where the final one or two parameters may be omitted.  For an input- or
output-only stream, one of the Buffer parameters may also be omitted.  The two
Buffer types may be different.
*/
...
func OpenStream(p StreamParameters, args ...interface{}) (*Stream, error) {
	...
	cb := (*C.PaStreamCallback)(unsafe.Pointer(C.CgoStreamCallback))
	if !s.callback.IsValid() {
		cb = nil
	}
	paErr := C.Pa_OpenStream(&s.paStream, s.inParams, s.outParams,
		C.double(p.SampleRate), C.ulong(p.FramesPerBuffer), C.PaStreamFlags(p.Flags),
		cb, unsafe.Pointer(s.id))
	if paErr != C.paNoError {
		delStream(s)
...
func streamCallback(inputBuffer, outputBuffer unsafe.Pointer, frames C.ulong,
	...
	s.timeInfo = StreamCallbackTimeInfo{duration(timeInfo.inputBufferAdcTime),
		duration(timeInfo.currentTime), duration(timeInfo.outputBufferDacTime)}
